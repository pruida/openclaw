...
import { createLLMAuditLogger } from "../../llm-audit-log.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      const llmAuditLogger = createLLMAuditLogger({
        env: process.env,
        runId: params.runId,
        sessionId: activeSession.sessionId,
        sessionKey: params.sessionKey,
      ...
      if (llmAuditLogger) {
        activeSession.agent.streamFn = llmAuditLogger.wrapStreamFn(activeSession.agent.streamFn);
      }
      ...
      try {
        ...
        llmAuditLogger?.recordResponse(messagesSnapshot, promptError);
