...
// Serve files generated by the `ppt` tool (and any future tool that writes to
// ~/.openclaw/downloads/<token>/<filename>). The 32-hex token is the access
// control: anyone holding the URL can fetch, but the token is generated with
// 16 random bytes per file so guessing is computationally infeasible. We
// re-resolve the absolute path and verify it stays under the downloads root,
// blocking path-traversal even though the regex already disallows the danger
// chars. No auth header is required so taps from chat work without state.
async function handleDownloadsRequest(
  req: IncomingMessage,
  res: ServerResponse,
  requestPath: string,
): Promise<boolean> {
  const m = /^\/downloads\/([0-9a-f]{32})\/([^/?#]+)$/.exec(requestPath);
  if (!m) {
    return false;
  }
  const token = m[1];
  ...
  const root = path.join(process.env.HOME ?? "", ".openclaw", "downloads");
  const requested = path.resolve(root, token, filename);
  ...
  res.setHeader("Content-Type", inferDownloadMime(filename));
...
// Serve files generated by the `site` tool. Different from /downloads/* in
// two ways: (a) Content-Type matches the file extension so the browser
// renders rather than offering a save dialog, and (b) NO Content-Disposition
// so HTML/CSS/JS load inline. Path-traversal protection identical to the
// downloads route — we re-resolve the absolute path and verify it's still
// inside ~/.openclaw/sites/<token>/.
async function handleSiteRequest(
  req: IncomingMessage,
  res: ServerResponse,
  requestPath: string,
): Promise<boolean> {
  const m = /^\/sites\/([0-9a-f]{32})(?:\/(.*))?$/.exec(requestPath);
  if (!m) {
    return false;
  }
  const token = m[1];
  let relPath = m[2] ? decodeURIComponent(m[2]) : "";
  ...
  const root = path.join(process.env.HOME ?? "", ".openclaw", "sites", token);
  ...
  res.setHeader("Content-Type", inferSiteMime(requested));
  res.setHeader("Content-Length", String(stat.size));
  // Short cache so iterating on a generated site doesn't show stale assets,
  // but the browser doesn't refetch on every navigation either.
...
function inferSiteMime(filePath: string): string {
...
function inferDownloadMime(filename: string): string {
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      // Downloads + sites stages must come BEFORE control-ui-http: that one
      // is an SPA fallback and will intercept any path that's not its own,
      // returning the control-UI index.html instead of our generated file.
      requestStages.push({
        name: "downloads",
        run: () => handleDownloadsRequest(req, res, requestPath),
      });

      requestStages.push({
        name: "sites",
        run: () => handleSiteRequest(req, res, requestPath),
      });
